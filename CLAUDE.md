...
8. On startup (and after `/clicksorted admin reload`), `UpdateChecker.restart()` fires an async best-effort Modrinth API call (when `check_for_updates: true`) that logs a console notice if a newer release exists, then rearms the recurring schedule (`check_for_updates_interval_hours`, default 24, clamped to a minimum of `MainConfig.MIN_UPDATE_CHECK_INTERVAL_HOURS`); the recurring task is cancelled via `stop()` on disable. The outcome of the most recent completed check is cached on the instance (`isUpdateAvailable()` / `getLatestVersion()`, published via `volatile` since the check runs on the async scheduler thread) rather than re-queried, so join-time consumers don't hit Modrinth on every login. On `PlayerJoinEvent`, `UpdateNotifyListener` notifies any player holding `clicksorted.admin.notify.update-available` in chat when that cache says a newer release is available and `notify_admins_on_update: true` (config default true; has no effect when `check_for_updates` is false).
...
| `UpdateChecker` | update | Best-effort async Modrinth API check; logs a console notice when a newer release exists. `restart()` (the single enable/reload entry point) fires a gated immediate check then calls `reschedule()`, which (re)arms a recurring check per `check_for_updates_interval_hours`; `stop()` cancels it. Caches the outcome of the most recent completed check (`isUpdateAvailable()` / `getLatestVersion()`, `volatile`-published) for join-time consumers. |
